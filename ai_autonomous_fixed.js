...
function createEnemy(x, z, formationId) {
    ...
    enemyGroup.userData = {
        ...
        moveSpeed: 0.07 + Math.random() * 0.03, // Patrol speed 0.07-0.1
        huntingSpeed: 0.33, // Fixed hunting speed
        // Hunting behavior
        isHunting: false,
        detectionRadius: 15,
        lastPlayerSeen: 0,
...
function updateEnemyAI() {
    ...
    window.targets.forEach(enemy => {
        ...
        // SIMPLE SHOOTING SYSTEM - ONLY SHOOT IF DETECTED/ALERTED!
        if (ai.isAlerted && distanceToPlayer <= AI_CONFIG.ATTACK_RANGE && currentTime - ai.lastShoot > 2700) {
...
function updateSimpleBullets() {
    ...
    for (let i = window.simpleBullets.length - 1; i >= 0; i--) {
        ...
        if (distToPlayer < 2.5) {
            ...
            // Damage player - ensure we're using the global variable
